...
const form = useForm({
    ...
    descripcion: '',
    id_curso: '',
    asignatura: props.asignaturas.length > 0 ? props.asignaturas[0] : '',
...
<template>
    <modal-overlay @cerrar="$emit('close')">
        <modal-content>
            <template #default>
                <modal-form @submit.prevent="crearObs">
                    <div class="flex">
                        <!-- Selección de tipo-->
                        ...
                        <div class="mb-4 w-80 flex ml-4" >
                            ...
                            <Multiselect v-else class="text-darkblue-800 w-60" track-by="id" label="nombre"
                                v-model="selectedPlantel" placeholder="Seleccionar docente"
